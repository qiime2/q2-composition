...
export class DivergingBarplot {
  ...
  updatePlotHeight() {
    this.dimensions.plotHeight =
      this.data.length * this.dimensions.barHeight * this.dimensions.barPadding;
  ...
  drawErrorBar(path: d3.Path, d: ViewRecord, i: number): d3.Path {
    ...
    const startY = this.yScale(i.toString())! + this.dimensions.barHeight / 2;
  ...
  drawPlot(transition: boolean) {
    ...
    const drawLabels = (selection: any) => {
      selection
        ...
        .attr(
          "y",
          (d, i) => this.yScale(String(i)) + this.yScale.bandwidth() / 2,
        )
    ...
    if (transition) {
      labelSelection.transition().duration(400).call(drawLabels.bind(this));
    } else {
